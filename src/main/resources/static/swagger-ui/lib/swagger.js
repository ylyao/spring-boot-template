...
SwaggerApi.prototype.buildFrom1_1Spec = function (response) {
    log("This API is using a deprecated version of Swagger!  Please see http://github.com/wordnik/swagger-service/wiki for more info");
    if (response.apiVersion != null)
        this.apiVersion = response.apiVersion;
    this.apis = {};
    this.apisArray = [];
...
/*
 * ShredHttpClient is a light-weight, node or browser HTTP domain
 */
...
ShredHttpClient.prototype.execute = function (obj) {
    ...
    var transformError = function (error) {
        var out = {
            // Default to a status of 0 - The domain will treat this as a generic permissions sort of error
            status: 0,
        ...
        if (error.code) {
            ...
            if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
                // We can tell the domain that this should be treated as a missing resource and not as a permissions thing
                out.status = 404;
            }
        }
